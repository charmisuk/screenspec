...
const MUTS = [
  ...
  { id: "chip-always-saved", only: "[chip]", why: "저장 안 된 상태를 «저장됨» 으로 그린다 — 글이 날아가는 종류의 거짓말",
    find: "                  : edDirty                 ? \"dirty\"",
    to:   "                  : false                   ? \"dirty\"" },
  { id: "chip-both-shown", only: "[chip]", why: "상태 글줄과 단추를 다시 둘 다 보여 준다 (합친 것이 풀린다)",
    find: "      edStat.hidden = asBtn;",
    to:   "      edStat.hidden = false;" },
  ...
  { id: "mm-png-off", only: "[머메이드]", why: "그림에서만 순서도가 사라진다 (#98)",
    ...
    to:   "      if (false) {\n        const got = MM_SVG.get(d);" },
  { id: "mm-no-stage", only: "[머메이드]", why: "재는 칸을 안 넘긴다 — 액자 모드에서 선 글자 순서도가 안 그려진다 (#115 원래 버그)",
    find: "mm.render(\"ss-mm-\" + run + \"-\" + i, String(d.code || \"\"), stage)",
    to:   "mm.render(\"ss-mm-\" + run + \"-\" + i, String(d.code || \"\"))" },
  { id: "mm-stage-hidden", only: "[머메이드]", why: "재는 칸이 액자 모드의 숨김 규칙에 걸린다 (#115)",
    find: "h(\"div\", { class: \"ss-mm-stage\", \"data-ss-ignore\": \"1\" })",
    to:   "h(\"div\", { class: \"ss-mm-stage\" })" },
  { id: "cap-anim-replays", only: "[그림] 열린 패널", why: "복사본에서 끝난 애니메이션이 다시 돈다 — 채움 없는 것은 첫 프레임(투명)으로 찍힌다 (#116)",
    find: "if (named) css.animation = \"none\";",
    to:   "if (false) css.animation = \"none\";" },
  { id: "cap-anim-unfrozen", only: "[그림] 열린 패널", why: "지금 걸린 효과의 값을 안 박는다 — forwards 로 보이던 것이 기본값(투명)으로 돌아간다 (#116)",
    find: "            if (KF_META[k]) return;\n            const kk",
    to:   "            return;\n            const kk" },
  { id: "cap-fixed-float", only: "[그림] 열린 패널", why: "fixed 를 못 박지 않는다 — 그림 틀 기준으로 풀려 머리말을 덮는다 (#116)",
    find: "          if (!x.fix || !c || !target.contains(c)) return;",
    to:   "          if (true) return;" },
  { id: "cap-freeze-late", only: "[그림] 열린 패널", why: "wrap 에서 옮긴 뒤에 안 박는다 — 옮기는 순간 등장 애니메이션이 다시 돌아 패널이 빠진다 (#116 원래 버그)",
    find: "        live.forEach((x) => { freeze(x.el)(x); applyState(x.el, x.st); }); /* 옮긴 «직후»",
    to:   "        live.forEach((x) => { applyState(x.el, x.st); }); /* 옮긴 «직후»" },
  { id: "cap-frozen-stays", only: "[그림] 열린 패널", why: "그림 때문에 박은 값을 안 푼다 — wrap 의 살아 있는 앱에 !important 가 남는다 (#116)",
    find: "          restoreSticky(); restoreFrozen(); restoreSrc(); box.remove();",
    to:   "          restoreSticky(); restoreSrc(); box.remove();" },
  { id: "wrap-height-lost", only: "[그림] wrap 에서", why: "뽑은 뒤 시트 높이를 빈 값으로 돌린다 — 기기 프레임이 쪼그라든다 (#119 원래 버그)",
    find: "          sheet.style.height = was.h;",
    to:   "          sheet.style.height = \"\";" },
  { id: "export-restore-height", only: "[export]", why: "뽑은 뒤 시트 높이를 빈 값으로 돌린다 — 예전 「원상 복귀」 검사는 이걸 정답으로 통과시켰다 (#119)",
    find: "          sheet.style.height = was.h;",
    to:   "          sheet.style.height = \"\";" },
  { id: "wrap-marks-deleted", only: "[그림] wrap 에서", why: "「번호 끔」 이면 살아 있는 번호 층을 지운다 — 화면에서 번호가 사라진다 (#119 원래 버그)",
    find: "forEach((n) => { layersHid.push([n, n.style.display]); n.style.display = \"none\"; });",
    to:   "forEach((n) => n.remove());" },
  { id: "wrap-marks-stay-hidden", only: "[그림] wrap 에서", why: "숨긴 번호 층을 안 되돌린다 (#119)",
    find: "          layersHid.forEach(([n, d]) => (n.style.display = d));",
    to:   "" },
  { id: "area-never", only: "[그림] 영역", why: "영역 상자를 안 그린다 — 번호만으로는 어디까지인지 모른다 (#117 원래 상태)",
    find: "      if (!ml || !rects.length) return null;",
    to:   "      return null;" },
  { id: "area-ignores-major", only: "[그림] 영역", why: "「주요 항목만」 인데 상자는 전부 친다 — 번호와 상자가 어긋난다 (#117)",
    find: "        if (noMark(it.spec) || (opt.major && !isMajor(it.spec))) return;",
    to:   "        if (noMark(it.spec)) return;" },
  { id: "area-without-marks", only: "[그림] 영역", why: "번호를 껐는데 상자만 남는다 — 상자는 번호의 하위다 (#117)",
    find: "const areas = opt.markers !== false && opt.areas ? capAreaRects(opt) : null;",
    to:   "const areas = opt.areas ? capAreaRects(opt) : null;" },
  { id: "area-default-off", only: "[그림] 영역", why: "영역 상자가 기본으로 꺼져 있다 — PM 이 원한 기본이 아니다 (#117)",
    find: "mark: true, area: true, major: false",
    to:   "mark: true, area: false, major: false" },
  { id: "area-sketch-lies", only: "[그림] 영역", why: "스케치가 상자 설정을 안 따라간다 (#117)",
    find: "      prDlg.querySelectorAll(\".ss-pr-p-area\").forEach((el) => { el.hidden = !(prCfg.mark && prCfg.area); }); /* #117 */",
    to:   "" },
  { id: "area-leaks-live", only: "[그림] 영역", why: "wrap 의 살아 있는 번호 층에 상자가 남는다 — 뽑고 나면 화면에 상자가 붙어 있다 (#117)",
    find: "          if (areaLayer) areaLayer.remove();\n",
    to:   "" },
  { id: "area-radius-flat", only: "[그림] 영역", why: "상자 모서리가 대상의 둥글기를 안 따른다 (#117)",
    find: "\"px;border-radius:\" + Math.max(4, a.rad) + \"px\"",
    to:   "\"px;border-radius:4px\"" },
  { id: "area-dialog-drops", only: "[그림] 영역", why: "대화상자가 영역 상자 선택을 그림에 안 넘긴다 (#117)",
    find: "        areas: !!(prCfg.mark && prCfg.area),",
    to:   "        areas: false," },
  { id: "area-cleared-with-marks", only: "[그림] 영역", why: "번호를 껐다 켜면 상자 선택이 꺼져 있다 — 매번 다시 체크해야 한다 (#117)",
    find: "          if (k === \"mark\" && !e.target.checked) prCfg.major = false;",
    to:   "          if (k === \"mark\" && !e.target.checked) { prCfg.major = false; prCfg.area = false; }" },
  { id: "ol-view-dot", only: "[번호 목록]", why: "뷰어가 번호 목록에 번호를 안 붙인다 — 불릿과 구별이 안 된다 (#118)",
    find: "        kind === B_OL ? '<span class=\"ss-b-num\">' + (n.ol || 1) + \".</span>\" : \"\") +",
    to:   "        kind === B_OL ? \"\" : \"\") +" },
  { id: "ol-run-no-reset", only: "[번호 목록]", why: "다른 종류가 끼어도 번호를 이어 센다 — 두 목록이 한 목록이 된다 (#118)",
    find: "          if (blkKind(b) === B_OL) node.ol = ++run; else run = 0;",
    to:   "          if (blkKind(b) === B_OL) node.ol = ++run;" },
  { id: "ol-hidden-counts", only: "[번호 목록]", why: "가려진 줄(기획 보기의 개발 줄)까지 센다 — 보이는 번호가 건너뛴다 (#118)",
    find: "        if (keep) {\n          const node = { b: b, depth: depth, path: p };\n          if (blkKind(b) === B_OL) node.ol = ++run; else run = 0;",
    to:   "        if (blkKind(b) === B_OL) run++; else run = 0;\n        if (keep) {\n          const node = { b: b, depth: depth, path: p };\n          if (blkKind(b) === B_OL) node.ol = run;" },
  { id: "ol-png-plain", only: "[번호 목록]", why: "그림 속 표에서 번호 목록이 번호 없이 나간다 (#118)",
    find: "(kind === B_OL ? '<span class=\"ss-pr-num\">' + (n.ol || 1) + \".</span>\" : \"\") +",
    to:   "\"\" +" },
  { id: "ol-text-ul", only: "[번호 목록]", why: "설명 복사가 번호 목록을 <ul> 로 낸다 — 붙인 곳에서 진짜 번호 목록이 아니다 (#118 원래 증상)",
    find: "          const tag = isOl ? \"ol\" : \"ul\";",
    to:   "          const tag = \"ul\";" },
  { id: "ol-md-dash", only: "[번호 목록]", why: "설명 복사 글(마크다운)이 번호 목록을 - 로 낸다 (#118)",
    find: "m: ind + (kind === B_OL ? num + \". \" : \"- \") + dev",
    to:   "m: ind + \"- \" + dev" },
  { id: "ol-slash-missing", only: "[번호 목록]", why: "슬래시 메뉴에 「번호 목록」 이 없다 (#118)",
    find: "      { k: \"ol\", ico: \"1.\", nm: \"번호 목록\", key: \"1.\" },",
    to:   "" },
  { id: "ol-shortcut-off", only: "[번호 목록]", why: "«1.» + 스페이스가 번호 목록이 안 된다 — 글자로 남는다 (#118)",
    find: "        else if (k === \" \" && /^\\d+\\.$/.test(edEl.textContent)) { eat(); edEl.textContent = \"\"; edPickUp(); edSetKind(B_OL); }",
    to:   "" },
  { id: "ol-enter-breaks", only: "[번호 목록]", why: "Enter 가 번호를 안 잇는다 — 다음 줄이 그냥 글 줄이다 (#118)",
    find: "        nb.kind = cur.kind === B_BULLET || cur.kind === B_OL ? cur.kind : B_TEXT;",
    to:   "        nb.kind = cur.kind === B_BULLET ? B_BULLET : B_TEXT;" },
  { id: "ol-enter-trapped", only: "[번호 목록]", why: "빈 번호 줄에서 Enter 가 또 번호 줄을 만든다 — 목록에 갇힌다 (#118)",
    find: "        if (cur.kind === B_OL && !String(cur.t || \"\").replace(/<[^>]*>/g, \"\").trim()) {",
    to:   "        if (false) {" },
  { id: "miss-warns-early", only: "[docs] 비동기", why: "조용해지자마자 누락 경고를 낸다 — 늦게 오는 조회 화면이 «없는 것» 취급된다 (#23)",
    find: "        if (!final) return; /* 아직 상한 전",
    to:   "        if (false) return; /* 아직 상한 전" },
  { id: "job-not-counted", only: "[내보내기]", why: "그림 굽기를 «하던 일» 로 안 센다 — busy() 가 0 이라 시험이 덜 구워진 판을 본다 (#120)",
    find: "    function exportImage(opt) { return ssJob(exportImageRun(opt)); }",
    to:   "    function exportImage(opt) { return exportImageRun(opt); }" },
  { id: "mode-moves-frame", only: "[상태]", why: "모드를 바꿀 때 프레임을 다시 붙인다 — 액자가 다시 로드돼 앱 상태가 사라진다 (#121 원래 버그)",
    find: "      core.soloRoots(m === \"doc\");",
    to:   "      docHolder.appendChild(frame);\n      core.soloRoots(m === \"doc\");" },
  { id: "state-not-copied", only: "[상태] 보이는", why: "보이는 상태를 복사본에 안 옮긴다 — 체크해 둔 칸이 그림에서 빈 칸이 된다 (#121 원래 버그)",
    find: "        if (!t || !st) return;",
    to:   "        if (!t || !st || 1) return;" },
  { id: "state-canvas-flat", only: "[상태] 보이는", why: "캔버스를 안 바꿔 끼운다 — 그림 틀이 캔버스를 안 그려 백지가 된다 (#121)",
    find: "        capCanvasSwap(clone); /* 캔버스 → 같은 그림의 div",
    to:   "        0; /* 캔버스 → 같은 그림의 div" },
  { id: "state-scroll-top", only: "[상태] 보이는", why: "스크롤 자리를 안 그린다 — 그림은 늘 맨 위를 보여 준다 (#121)",
    find: "        if (st.scroll) { /* 스크롤 자리도 마크업에 없다",
    to:   "        if (false) { /* 스크롤 자리도 마크업에 없다" },
  { id: "state-pseudo-off", only: "[상태] 보이는", why: "걸린 상태(:hover·:focus)의 규칙을 다시 안 적는다 — 올린 단추가 민낯이 된다 (#121)",
    find: "  function pseudoRule(text) {\n    let has = false;",
    to:   "  function pseudoRule(text) {\n    return \"\";\n    let has = false;" },
  { id: "state-stays", only: "[상태] 보이는", why: "그림 때문에 붙인 표식·속성을 앱에 남긴다 (#121)",
    find: "          stUndo.reverse().forEach((f) => f());",
    to:   "" },
  { id: "move-loses-scroll", only: "[상태] 보이는", why: "wrap 에서 뽑고 나면 안쪽 스크롤 자리가 풀린다 — 옮겼다 온 대가다 (#121)",
    find: "              x.el.scrollTop = x.st.scroll.t;",
    to:   "" },
  { id: "move-loses-focus", only: "[상태] 보이는", why: "wrap 에서 뽑고 나면 적던 칸에서 커서가 튕긴다 (#121)",
    find: "              try { wasFocus.focus({ preventScroll: true }); } catch (e) { /* 못 받는 요소 */ }",
    to:   "" },
  ...
  { id: "brief-marker-on", only: "[개요]", why: "개요에도 마커를 만든다 (#82 · #107 에서 섹션까지 같은 줄)",
    find: "        if (noMark(it.spec)) return; /* 개요·섹션은 화면 위 요소가 아니다",
    to:   "        if (false) return; /* 개요·섹션은 화면 위 요소가 아니다" },
  ...
  { id: "brief-slash-off", only: "[개요]", why: "슬래시 메뉴에 화면 개요가 안 나온다 (#82)",
    ...
    to:   "      const scr = [];" },
  { id: "brief-breaks-root", only: "[개요]", why: "개요가 있으면 화면 연결이 끊긴다 (#82 · #107 에서 섹션까지 같은 줄)",
    find: "        const sps = (sc.specs || []).filter((sp) => !noMark(sp));",
  ...
  { id: "marker-px-raw", only: "[move]", why: "마커 좌표를 안 자르고 써 추적 루프가 영영 안 멎는다 (#104 가 깨운 잠복 버그)",
    ...
    to:   "        const px = (v) => v + \"px\";" },
  { id: "proto-never-shrinks", only: "[배율]", why: "프로토타입만 실물 크기를 고집한다 — 「맞춤」 버튼 시절로 되돌린다 (#105)",
    find: "      scale = fitScale(stage); /* 두 모드가 같은 상자",
    to:   "      scale = document.body.classList.contains(\"ss-mode-doc\") ? fitScale(stage) : 1; /* 두 모드가 같은 상자" },
  { id: "zoom-over-one", only: "[배율]", why: "들어가는 시트를 «키워서» 채운다 — 줄이는 것이 아니라 늘린다 (#105)",
    find: "      return Math.max(0.2, Math.min(1, b.w / sheetW, bound ? b.h / sheetH : 1));",
    to:   "      return Math.max(0.2, Math.min(b.w / sheetW, bound ? b.h / sheetH : 1e9));" },
  { id: "scale-always-said", only: "[배율]", why: "안 줄었는데도 «100%» 를 붙여 말한다 — 할 말이 없을 때 떠든다 (#105)",
    find: "        (scale < 0.995 ? ' <span class=\"ss-sc\">\\u00B7 ' + Math.round(scale * 100) + \"%</span>\" : \"\"));",
    to:   "        (' <span class=\"ss-sc\">\\u00B7 ' + Math.round(scale * 100) + \"%</span>\"));" },
  { id: "scale-never-said", only: "[배율]", why: "줄여 놓고 말을 안 한다 — 왜 작아졌는지 알 길이 없다 (#105)",
    find: "      setHTML(wpx, sheetW + \"\\u00D7\" + sheetH + /* 셋 다 숫자다 — 넣을 수 있는 문자열이 없다 */",
    to:   "      setHTML(wpx, sheetW + \"\\u00D7\" + sheetH + \"\" || /* 죽은 가지 */" },
  ...
  { id: "holder-phantom-space", only: "[배율]", why: "축소해도 홀더가 원래 크기만큼 자리를 먹어 빈 곳으로 스크롤된다 (#104)",
    find: "        sty(holder, \"width\", scale === 1 ? \"\" : Math.ceil(sheetW * scale) + \"px\");",
    to:   "        sty(holder, \"width\", \"\");" },
  { id: "frame-squeezed", only: "[배율]", why: "프레임이 홀더를 따라 눌려 시트와 손잡이가 어긋난다 (#104)",
    find: "      sty(frame, \"width\", sheetW + \"px\");",
    to:   "      sty(frame, \"width\", \"\");" },
  ...
  { id: "split-scrollers", only: "[무대]", why: "가로는 상자가 세로는 문서가 맡던 옛 배치로 되돌린다 (#102)",
    find: "  .ss-docmode{display:flex;flex-direction:column;position:fixed;top:50px;left:0;right:0;bottom:0;z-index:9000}",
    to:   "  .ss-docmode{display:flex;flex-direction:column;padding:74px 16px 60px;overflow-x:auto}" },
  { id: "stage-pad-zero", only: "[무대]", why: "프로토타입 여백을 0 으로 — 손잡이가 잘린다 (#102 제안 ②)",
    find: "    --ss-stage-pad:24px}",
    to:   "    --ss-stage-pad:0px}" },
  { id: "preset-dead", only: "[무대]", why: "프리셋을 눌러도 크기가 안 돌아온다 — «누르면 돌아갈 자리» 가 사라진다 (#105)",
    find: "      applySize(d.w, d.h);\n    }\n    seg.addEventListener",
    to:   "      if (!sheetW) applySize(d.w, d.h);\n    }\n    seg.addEventListener" },
  { id: "window-resizes-sheet", only: "[무대]", why: "창 크기 변경이 시트 «크기» 를 바꾼다 — 기준 폭이 사람마다 달라진다 (#105)",
    find: "    window.addEventListener(\"resize\", () => { layout(); core.placeMarkers(); });",
    to:   "    window.addEventListener(\"resize\", () => { const b = inner(protoWrap); if (b.w > 0) applySize(b.w, b.h); layout(); core.placeMarkers(); });" },
  ...
  { id: "text-drops-blocks", only: "[설명]", why: "표·순서도가 글에서 조용히 사라진다 — 이 도구에서 제일 나쁜 실패 (#108)",
    find: "          if (r.block) { close(); H2.push(r.h); M2.push(r.m); return; }",
