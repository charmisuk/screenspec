...
⚠ **지금은 이 약속을 못 지킨다.** 정의(`defs`)가 «평평한 목록 + 절대 깊이 숫자» 라서 부모가 저장되지 않고 «앞쪽에서 깊이가 하나 작은 첫 블록» 으로 **계산**된다. 앞에 무엇이 끼거나 빠지면 뒤 블록의 부모가 조용히 바뀐다.

`node scripts/qa-drag.js` 가 그 빚을 센다 — **지금 272건 중 16건**. 트리(명시적 부모)로 옮기면 0 이 되어야 하고, 그것이 그 작업의 합격 기준이다.
