...
## 고치는 동안에는 e2e 를 통째로 돌리지 않는다 (PM 2026-08-31)

- 재 봤다: lint 0초 · **e2e 158초** · smoke 22초. e2e 158초 중 **112초가 고정 대기**다
- 고치는 동안에는 `node tests/e2e.js --only <섹션>` (수 초). 섹션 목록은 `--list`
- **커밋 직전에만 전체 1회.** 커밋 게이트는 lint + e2e 전체 + smoke 그대로다
