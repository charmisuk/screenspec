...
- **갈림길이었던 항목에는 `major: true` 를 준다.** 한 정의서에서 상위기획(갈림길만)과 상세기획(전부) 두 그림이 나온다. 내보내기의 「주요 항목만」이 이 표시를 보고 그림의 번호와 설명 표를 함께 거른다. `layer`(누가 쓰나)와 **다른 축**이다 — 이건 「어느 깊이인가」다. 안 쓰면 키가 안 생긴다.
...
<Script src="https://cdn.jsdelivr.net/gh/charmisuk/screenspec@v0.35.0/screenspec.js" strategy="afterInteractive" />
```
...
**뽑아서 넘기기** — 툴바의 «내보내기» 로 화면 한 장을 PNG 로 뽑는다(머리말 네 조각·번호·영역 상자·기능 설명 표·번호 색을 고른다).
...
| (블록) `ol` | 번호 목록 | 순서나 가짓수가 뜻인 목록 (「이 화면이 하는 일 두 가지」 · 「역할 추가의 세 단계」) | 줄에 `kind:"ol"`. 이어지는 `ol` 줄이 한 목록(1. 2. 3.)이고 다른 종류가 끼면 다시 1. **①·1) 글자로 흉내 내지 않는다** — 컨플·노션에 붙이면 진짜 번호 목록이 아니다 (#118) |
...
| `section` | 섹션 | 화면 위를 가리키지 않는 항목을 **그 자리에** 둘 때 (권한 표·공통 규칙처럼 모든 화면에 붙는 것) | `target`·`n` 을 적지 않는다. 번호 항목으로 두면 «마커 없는 번호» 가 되고 누락 경고가 난다. `/` → 「섹션」 |
