...
툴바(wrap) 또는 모드 알약(overlay)의 **「내보내기」** 를 누른다.
설명 패널이 아니라 툴바에 있는 이유는, 내보내기가 **패널이 아니라 화면 전체**에 작용하기 때문이다.

| 형식 | 무엇이 나오나 | 언제 |
|---|---|---|
| **이미지 (PNG)** — 기본 | 화면 그림 한 장 | 컨플·노션에 **붙여넣을** 때. 실무에서는 그림을 붙이고 설명은 옆에 직접 쓴다 |
| **인쇄 · PDF** | 문서 한 장 | 결재·제출처럼 «문서» 가 필요할 때 |

| 선택 | 기본값 | 뜻 |
|---|---|---|
| **번호(마커)** | 켬 | 끄면 프로토타입 그림만 — 번호를 옆에 직접 매길 때 |
| **머리말** | 켬 | 화면 ID · 화면명 · 경로 · 생성 일시를 그림 위에 함께 굽는다 |
| **기능 설명 표** | **끔** | 켜면 그림 아래에 표까지 한 장으로 |
| **레이어** | 전체 | 기획만 / 개발만. [개발 정의](#개발-정의-레이어-layer)가 있는 문서에서만 |
...
이 라이브러리 전체보다 크다. 대신 두 가지를 못 한다 — **조용히 넘기지 않고 알린다.**
...
- **`mode: "frame"` 은 이미지로 못 뽑는다.** 액자 안은 다른 문서다 — 이 모드에서는 인쇄만 제공한다

인쇄(PDF)의 세부는 위 표의 선택을 그대로 따르며, 표 행은 페이지 중간에서 잘리지 않고 여러 장이면 장마다 머리행이 다시 붙는다.
...
window.ScreenSpec.exportImage({ markers, head, table, layer, scale })  // 화면을 PNG 로 내려받는다 (wrap 전용)
