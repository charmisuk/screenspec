...
사양의 값에는 대개 원본이 있다 — 정책 문서, 운영 실측, API 명세. 그 근거가 정의서 밖에만 있으면 읽는 사람이 낡은 문서를 최신으로 오인한다. 본문에 링크를 섞으면 사양이 밀려나므로 **각주**로 단다: 본문 침범은 위첨자 한 글자다.
...
- **번호를 누르면 원본으로 간다** (새 탭). 화면 발치에 「출처」 목록이 서서 «이 화면이 무엇에 근거하나» 를 한눈에 보여준다
