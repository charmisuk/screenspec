...
| **저장** | 오른쪽 위 **자동저장 켜기** → 이 HTML 파일을 한 번 고른다. 그 뒤로는 손이 멈출 때마다 저장되고 「저장됨」이 뜬다 |
...
| 어디 있는 영역인지 | 아무것도 안 적는다 | 실제 좌표로 「상단 · 전체폭」 같은 위치 힌트가 제목 옆에 자동으로 붙는다 |
...
| 찍은 번호가 다시 열면 사라진다 | 저장을 안 한 것. 자동저장을 켜거나 「설명 복사」로 원본에 붙여넣는다 |
