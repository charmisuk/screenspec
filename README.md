...
**주소 고르기** (원본용): `@0`은 릴리스가 나올 때마다 자동 반영, `@v0.17.0`처럼 태그를 박으면 그 시점으로 고정.
...
단, 이건 **숨김이지 보안이 아니다** — 정의 텍스트는 페이지 소스에 남아 소스를 열면 읽힌다. 정말 나가면 안 되는 내용이면 정의를 지운 사본을 따로 만든다.
...
| 특정 상태에서만 서는 버튼 | `optional: true` | 지금 화면에 없어도 누락 경고에서 빠지고 패널에 「현재 미표시」 |
