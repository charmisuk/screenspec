...
| **`/` 메뉴** | **줄 어디에서나** 열린다 (글자 뒤에 쳐도 된다). 이어 쳐서 거르고, 고르면 `/` 와 거르려고 친 글자가 사라진다. 걸리는 것이 없으면 그냥 글자로 남는다 |
| **화면 전체 설명 넣기** | 빈 줄에서 `/` → **화면 개요**. 번호 없이 목록 맨 위에 붙는다 (화면당 하나) |
| 층 바꾸기 | `Tab` / `Shift+Tab`. 또는 왼쪽 `⠿` 를 잡아 끌면서 좌우로 민다 |
| 순서 바꾸기 | `⠿` 를 잡아 끈다. 다른 번호로도 옮겨진다 |
| 블록 지우기 | 글자를 다 지우면 그 줄이 사라진다. 표처럼 비울 수 없는 블록은 **`⠿` 를 눌러 「삭제」**. 표 안의 빈 행은 `Backspace` |
| 줄 합치기 · 커서 이동 | 커서가 **화살표로 줄 사이를 그냥 흐른다** (표 칸으로도). 줄 맨 앞 `Backspace` 는 앞 줄과 합치고, 줄 끝 `Delete` 는 뒷줄을 끌어 붙인다. 표·순서도 앞에서는 멈춘다 |
...
| **앱에 심었을 때의 저장** | `save.write` 훅을 달면 앱(개발 서버)이 정의 소스 파일에 쓴다. 파일 고르기 창 없이 그대로 저장된다 → [모드](#어디에-붙이나-세-가지-모드) |
| 주소로 연 문서(사내 서버·깃허브 페이지) | 막지 않는다. 쓸 파일이 애초에 없기 때문이다. 대신 **첫 편집 때 한 번** 「이 문서는 파일에 못 씁니다 · 「설명 복사」로 원본에 옮기세요」를 알린다 |
| 로컬 파일인데 아직 연결 안 됨 | **고쳐지지 않는다.** 고치려 하면 패널 안에 연결 카드가 뜬다 — 고를 파일 이름과 폴더를 짚어 준다(프로토타입은 안 가린다). 남지 않을 수정을 하게 두지 않으려는 것이다 (주소로 받아 온 문서는 쓸 파일이 없으므로 막지 않는다) |
...
| 모드 | 대상 | 붙이는 법 |
|---|---|---|
| `wrap` (기본) | 단일 HTML 프로토타입 | 위 빠른 시작 그대로. 기기 폭 시뮬레이터(모바일 360×800 · PC 1920×1080 · 자동) 포함 |
...
**「자동」은 프레임을 지금 창에 맞춘다.** `PC 1920×1080`은 통계적 최다이지 내 모니터가 아니라서, 노트북에서 열면 프레임이 창을 넘고 **앱이 화면에 고정해 둔 패널·버튼이 창 밖에 놓인다**(밀면 보이지만, `focus()`·`scrollIntoView`·폼 검증의 「첫 오류로 이동」은 그리로 못 간다). 그럴 때 「자동」을 누르면 된다. 손잡이를 끌면 그 크기를 지키고, 「자동」을 다시 누르면 창에 맞춘다. 기본값은 그대로 `mobile`이다 — 화면정의서는 기준 폭이 있어야 「16열이 다 보인다」 같은 서술을 검증할 수 있다. 문서 기본값으로 쓰려면 `baseViewport: "auto"`.

**「맞춤」은 크기를 그대로 두고 축소해서 보여준다.** 「자동」과 **다른 축**이다 — 자동은 프레임 *크기*를 창에 맞춰(1920 → 1232) 앱의 미디어쿼리가 달라지고, 맞춤은 **1920인 채로 64%로 보기만** 한다. PC 화면을 노트북에서 검수할 때 필요한 쪽이다. 화면정의서 모드는 읽는 자리라 **늘 맞춰지고**(가로·세로 둘 다), 프로토타입 모드는 만지는 자리라 기본이 실물 크기다.

**폰처럼 좁은 화면에서는 상단 바가 접힌다** — 「모바일 | PC | 자동」 폭 시뮬레이터는 숨고(자기 화면이 곧 기기라 잴 것이 없다), 저장·설명 복사·내보내기는 오른쪽 **⋯** 안으로 들어간다. 저장할 것이 남아 있으면 ⋯ 위에 점이 뜬다.
...
받는 쪽(개발 서버)은 넘어온 텍스트로 정의 소스 파일을 바꾸면 된다. 자세한 것은 [저장을 호스트에](docs/config.md#저장을-호스트에-save).
...
| 조건·결과 짝을 표로 | 빈 줄에서 `/` → **표**. 또는 `{ kind: "table", head: [...], rows: [[...]] }` | 머리행이 있는 표. `Tab` 으로 칸 이동, 끝에서 `Enter` 면 행 추가. PNG·컨플루언스에도 표로 나간다 |
...
| 그중 **화면마다 안 적어도 되는 축** | `{ name: "로딩", common: "공통 컴포넌트가 처리" }` | 그 축은 화면별 ⚠ 를 안 내고, 화면 목록 발치에 **한 번** 나온다 |
| 조건 분기가 많은 항목을 순서도로 | `{ kind: "mermaid", code: "flowchart TD…" }` | 뷰어가 그린다 (그 문서만 mermaid.js 를 CDN 에서 지연 로드 · 못 그리면 코드가 바닥). 노션엔 코드로 붙이면 거기서 그려진다 |
| 값의 근거를 줄 끝 각주로 | 최상위 `sources: { kps: { label, href } }` + 줄에 `ref: "kps"` | 줄 **오른쪽 끝**에 번호 + 출처 이름(누르면 원본), 화면 발치에 「출처」 목록. 번호는 자동이다 |
...
| `@v0.29.0` 처럼 태그를 박는다 | 라이브러리가 그 판에 고정 | 오래 살릴 프로토타입 |
...
| 목차로 골라도 프로토타입이 그대로 | **앱(Next 등)**: 라우터가 `popstate` 를 안 듣는다 — `screenspec:screenchange` 를 받아 앱 라우터로 이동한다 → [화면 전환과 라우팅](docs/config.md#화면-전환과-라우팅-screenspecscreenchange). **단일 HTML**: 그 화면에 `root` 를 안 적은 것 (전환 알림에 「설명만」으로 뜬다) |
...
| 액자 모드인데 앱이 두 번 보인다 | v0.30 이전 판이다. 부팅 뒤에 최상위 DOM 이 바뀌면 바깥 문서의 앱이 안 감춰졌다 — 올리면 된다 |
| 앱이 화면에 고정한 버튼에 닿을 수 없다 | 프레임(예: PC 1920)이 창보다 커서 앱의 뷰포트가 창 밖으로 나간 것. 폭 시뮬레이터의 「자동」을 누르면 창에 맞는다 |
| 넘긴 문서가 나중에 달라져 있다 | CDN 한 줄짜리 원본을 넘긴 것. 전달은 항상 `.inline.html`로 |
| 상태 축을 화면마다 반복해서 적고 있다 | `checklist` 에 `{ name: "로딩", common: "공통 컴포넌트가 처리" }` 로 적는다. 그 축은 화면별 ⚠ 를 안 내고 화면 목록 발치에 한 번 나온다 |
...
```bash
node tests/e2e.js --list          # 섹션 47개 목록
node tests/e2e.js --only 자동저장  # 그 섹션만 (수 초 ~ 20초)
```

부분 실행은 결과 줄에 그렇게 찍힌다.
