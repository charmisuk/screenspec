...
- **초안을 받은 뒤 직접 고친다** — 정의서 패널에서 글 쓰듯 고친다. Enter로 새 줄, Tab으로 한 단 들어가고, `/`로 넣을 것을 고르고, Ctrl+Z로 되돌린다. 파일로 저장하며 코드를 안 봐도 된다 (`readonly`로 잠글 수도)
...
| [자체 완결 파일](#자체-완결-파일)로 만든다 | 라이브러리가 **파일 안에 들어간다** — 인터넷과 무관하게 영원히 그대로 | **남에게 넘기는 문서** |
