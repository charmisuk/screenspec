...
| **저장** | 오른쪽 위 **자동저장 켜기** → 이 프로토타입 HTML 파일을 한 번 고른다. 그 뒤로는 손이 멈출 때마다 알아서 저장되고 「저장됨」이 뜬다 (저장할 게 없으면 단추가 꺼진다) |
| 파일이 밖에서 바뀌면 | AI 가 같은 파일을 고치면 띠가 뜬다: **새로고침**(AI 것을 본다) 또는 **내 것 유지**. 그동안 자동저장은 멈춰서 AI 의 수정을 덮지 않는다 |
