...
```html
<script src="https://cdn.jsdelivr.net/gh/charmisuk/screenspec@v0.7.0/screenspec.js"></script>
```

주소 선택 — 용도에 따라:

- 박제(산출물 보존): `@v0.7.0` — 그 시점 그대로 영원히
...
- 모드 토글: 프로토타입 / 화면정의서
- **화면 목록(목차)**: 정의서 헤더의 화면 ID 클릭 → 전체 화면 목록 + 커버리지(N/M 정의됨), 행 클릭으로 화면 이동
