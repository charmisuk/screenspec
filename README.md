...
- **A4 문서로 뽑는다** — 화면 하나를 머리·프로토타입·항목 표·꼬리로 인쇄. 브라우저의 «PDF로 저장»이 곧 산출물
  ...
  window.SCREENSPEC = {
    ...
    specs: [
      { n: 1, target: "1", anno: "box", title: "상단 헤더", defs: [
        { t: "리포트 제목 + 조회 기간 표시" },
        { t: "기간은 이번 주 월요일 기준 자동 계산", why: "주간 회의가 월요일이라 그 주 기준이 자연스럽다" }
      ]},
...
1번 항목의 이유는 「↳ 이유:」로 사양과 분리되어 보이고, 2번 항목의 **▶ 동작 재생**을 누르면 실제로 저장 버튼이 눌린다.
...
**주소 고르기** (원본용): `@0`은 릴리스가 나올 때마다 자동 반영, `@v0.19.3`처럼 태그를 박으면 그 시점으로 고정.
