...
**주소 선택**: `@0`은 정식 릴리스가 나올 때마다 자동 반영(권장), `@v0.13.0`처럼 태그를 박으면 그 시점 그대로 고정된다.
...
| 클로드 대화창 미리보기에서 안 보인다 | 그 화면은 바깥 주소를 막는다(실측 확인). 코드를 `.html`로 저장해 브라우저에서 열거나, 라이브러리를 파일 안에 넣으면 된다 — `node scripts/inline.js 파일.html` |
...
자체 완결 파일 만들기: `node scripts/inline.js 프로토타입.html` — 라이브러리를 파일 안에 넣어 바깥 요청 없이 동작하게 한다 (미리보기 환경용).
