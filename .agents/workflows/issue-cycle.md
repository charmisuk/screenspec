---
description: 이슈 사이클 — GitHub 이슈를 중요순으로 1건씩 plan→dev→qa→commit 처리 (v1.1)
---

# Issue Cycle Protocol (v1.1)
...
- **accept 기본 체크** (v1.1 — 되돌림 3회 중 2회가 이 유형): 위치·레이아웃을 건드리면 "ScreenSpec 자체 fixed UI(헤더 48px·패널 400px·필)와 겹치지 않음" 을 반드시 넣는다. 예제(examples/*.html)를 고치는 이슈면 plan 에 **배치 제약**(어디에 둘지, 기존 e2e 가 의존하는 ID·문구 유지)을 적어 서브에이전트가 스스로 판단하지 않게 한다.
...
- **라우팅 (v1.1)**: 예상 변경 **≤40줄·파일 1~2개** → 메인 세션이 직접(`dev.model: "main"`). 서브에이전트는 컨텍스트를 처음부터 다시 읽어 소형 수정에는 배보다 배꼽이 크다(실측: 10줄 수정에 55~91k 토큰). 그 이상이거나 CSS·UI 를 여러 지점에서 만지면 `opus` 서브에이전트. 문서만이면 메인 직접.
